...
%
% Version 2C: Monte Carlo wobbling axis simulation
%            Consider rotation in 2D only
%            Model detection as Iy = I0_cos^2(phi) and Ix = I0_sin^2(phi)
%
%           Modified to to use tau / taud to find correlation
...
numberOfDyes = 10000;    % This many separate fluorophore simulations
numberOfTimePoints = 200; % Number of time points during camera exposure
...
TauDList = [0.01:0.01:20]'; % Define values of Tau-dashed
CorrelationData = zeros(size(TauDList,1),2);

for lpTau = 1:size(TauDList,1)
    TauD = TauDList(lpTau,1)
    %tIncrement =TauD/numberOfTimePoints; % Use a time increment smaller than T_C
    %sigma = sqrt( 2 * D_rot * tIncrement ); % See Harms 1999;
    % sigma = sqrt( TauD/(3)*1/numberOfTimePoints)
    sigma = sqrt( TauD/(3*numberOfTimePoints)); % Modified to use TauD.
    ...
    CorrelationData(lpTau,1) = TauD;
    CorrelationData(lpTau,2) = std(listPol);
end
%{
fg = figure(1);
hist(listPol,400)
