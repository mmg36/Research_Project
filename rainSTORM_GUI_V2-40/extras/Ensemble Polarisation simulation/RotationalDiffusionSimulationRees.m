...
numberOfTimePoints = 100; % Number of time points during camera exposure
...
D_rot = 6000; % k_B T / (8 pi eta a^3)... 6000 per second in 1 Pa s, 3 nm
            % Debye-Stokes-Einstein relation
            % Use 100 Pa s (gel) and 3 nm radius for GFP.

T_C = 1/(6*D_rot) % rotational correlation time
tIncrement =1.9445e-06; % 1 ms % Use a time increment smaller than T_C
...
[numberOfTimePoints*tIncrement/T_C, std(listPol)]
...
hist(listPol,20)
  ...
  title('Monte Carlo simulation of polarisation', 'fontSize', 18)
  set(gca, 'fontSize', 18)